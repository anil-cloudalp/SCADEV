				...
				{{#if isQtyEditable}}
					<div class="cart-lines-free-item-summary-container-qty-input">
						<button type="button" class="cart-lines-free-item-summary-quantity-remove" data-action="minus"
							{{#if isMinusButtonDisabled}}disabled{{/if}}>-</button>
						<input type="number" data-type="cart-free-item-quantity-input" name="quantity" id="quantity-{{lineId}}"
							class="cart-lines-free-item-summary-quantity-value quantity-{{lineId}}" value="{{line.quantity}}" min="1"
							max="{{qtyElegible}}" />
						<button type="button" class="cart-lines-free-item-summary-quantity-add" data-action="plus">+</button>
					</div>
				{{else}}
					<span class="cart-lines-free-item-summary-container-qty-value"
						data-type="cart-free-item-quantity-span">{{line.quantity}}</span>
				{{/if}}
			</div>
