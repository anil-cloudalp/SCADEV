...
<section class="footer-wrapper">
	<div class="footer-content-wrapper">
		...
		<div class="footer-social-news">
			{{#if extraFooterView.socialMediaTitle}}
				<h4>{{extraFooterView.socialMediaTitle}}</h4>
			{{/if}}
					...
					<div class="footer-content-social">
						<ul class="footer-content-social-list">
							{{#each extraFooterView.socialMediaLinks}}
								<li><a {{objectToAtrributes item}} title={{text}} data-hashtag="{{datahashtag}}"
										data-touchpoint="{{datatouchpoint}}" data-target="{{datatarget}}" target="_blank">{{#if icon}}<i
			...
			{{!-- <div class="footer-content-newsletter">
				{{#if extraFooterView.showLegacyNewsletter}}
					{{#if extraFooterView.newsletterText}}
						<p>{{extraFooterView.newsletterText}}</p>
					{{/if}}
						...
						{{#each extraFooterView.col1Links}}
							{{#if href}}
								<li class="footer-column-link-listitem"><a class="footer-column-link" {{objectToAtrributes item}}
										...
										data-touchpoint="{{#if datatouchpoint}}{{datatouchpoint}}{{else}}{{../currentTouchpoint}}{{/if}}"
										data-target="{{datatarget}}" target="{{datatarget}}">{{text}}</a></li>
							{{else}}
						...
						{{#each extraFooterView.col4Links}}
							{{#if href}}
								<li class="footer-column-link-listitem">
									<a class="footer-column-link" {{objectToAtrributes item}} data-hashtag="{{datahashtag}}"
										data-touchpoint="{{#if datatouchpoint}}{{datatouchpoint}}{{else}}{{../currentTouchpoint}}{{/if}}"
										data-target="{{datatarget}}" target="{{datatarget}}">{{text}}
									</a>
								</li>
							{{else}}
								<li class="footer-column-heading-listitem">
