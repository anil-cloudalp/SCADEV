<article class="overview-shipping">
	...
	<section class="overview-shipping-card">
		{{#if hasDefaultShippingAddress}}
			...
			<a class="overview-shipping-card-button-edit" href="/addressbook/{{shippingAddressInternalid}}"
				data-toggle="show-in-modal">{{translate 'Edit'}}</a>
		{{else}}
			<div class="overview-shipping-card-content">
				<p>{{translate 'We have no default address on file for this account.'}}</p>
			</div>
			<a href="/addressbook/new" class="overview-shipping-card-button-edit"
				data-toggle="show-in-modal">{{translate 'Create New Address'}}</a>
		{{/if}}
	</section>
</article>
