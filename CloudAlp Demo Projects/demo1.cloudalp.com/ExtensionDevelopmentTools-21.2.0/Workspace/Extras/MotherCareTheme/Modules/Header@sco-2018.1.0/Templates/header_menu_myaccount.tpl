...
<ul class="header-menu-myaccount">
	...
	<li class="header-menu-myaccount-item-level2" {{#if permission}}data-permissions="{{permission}}" {{/if}} {{#if
		permissionOperator}}data-permissions-operator="{{permissionOperator}}" {{/if}}>
		{{#if url}}
		<a class="header-menu-myaccount-anchor-level2" tabindex="-1" href="#" {{#if children.length}}data-action="push-menu"
			{{/if}} data-touchpoint="customercenter" data-hashtag="#{{url}}" name="{{name}}">
			{{translate name}}
			{{#if children.length}}<i class="header-menu-myaccount-menu-push-icon"></i>{{/if}}
		</a>
		{{else}}
		<span class="header-menu-myaccount-span-level2" tabindex="-1" {{#if children.length}}data-action="push-menu" {{/if}}
			name="{{name}}">
			{{translate name}}
			{{#if children.length}}<i class="header-menu-myaccount-menu-push-icon"></i>{{/if}}
		</span>
		...
		<ul class="header-menu-myaccount-level3">
			...
			<li {{#if permission}}data-permissions="{{permission}}" {{/if}} {{#if
				permissionOperator}}data-permissions-operator="{{permissionOperator}}" {{/if}}>
				<a class="header-menu-myaccount-anchor-level3" tabindex="-1" href="#" data-touchpoint="customercenter"
					data-hashtag="#{{url}}" name="{{name}}">{{translate name}}</a>
			</li>
			{{/each}}
		</ul>
		{{/if}}
