define('Demo.TrackMyOrders.Model', [
    'SC.Model',
    'SC.Models.Init',
    'Application',
    ...
    return SCModel.extend({
        ...
        list: function(data) {
            ...
            this.columns = {
                ...
                trackingnumbers : new nlobjSearchColumn('trackingnumbers')
            };

            console.warn("model this",JSON.stringify(this))
            console.warn("tracking number", JSON.stringify(this.columns))

            // if (this.data.selected_order_id) {
            //     this.filters.selected_order_id_operator = 'and';
            //     this.filters.selected_order_id = ['internalid', 'contains', this.data.selected_order_id];
            // }

            // if (this.data.selected_status) {
            //     console.warn("status filter", JSON.stringify(this.data.selected_status))
            //     // this.filters.selected_status_operator = 'and';
            //     // this.filters.selected_status = ['internalid', 'is', this.data.selected_status];
            // }
            ...
            if (this.data.selected_order_id) {
                this.filters.selected_order_id_operator = 'and';
                this.filters.selected_order_id = ['internalid', 'anyof', this.data.selected_order_id];
            }
            ...
            if (this.data.sort) {
                _.each(this.data.sort.split(','), function(column_name) {
                    if (self.columns[column_name]) {
                        console.warn("sort self.columns[column_name]", self.columns[column_name])
                        self.columns[column_name].setSort(self.data.order >= 0);
                    }
                });
            }

            if (this.data.statussort) {
                _.each(this.data.statussort.split(','), function(column_name) {
                    if (self.columns[column_name]) {
                        self.columns[column_name].setStatusSort(self.data.order >= 0);
                    }
                });
            }
            ...
            const records = _.map(
                ...
                console.warn("record",JSON.stringify(record))
                    ...
                    const result = {
                        ...
                        // Tracking number
                        trackingnumbers: record.getValue('trackingnumbers'),
                        // @class Transaction.Model.List.Result.Record
            ...
            this.postList();
                // console.warn("result.records length", this.results.records.length)
                this.statusSolution = _.groupBy(this.results.records, function(record){return record.status.name != self.data.statussort})
                // console.warn("result after changes ", JSON.stringify(this.results))
                // console.warn("status Solution ", JSON.stringify(this.statusSolution))
                if(!_.isUndefined(this.statusSolution.false)){
                this.results.records = this.statusSolution.false.concat(this.statusSolution.true)
                }
            return this.results;
        ...
        mapListResult: function(result,record) {
            console.warn("map list result", result)
