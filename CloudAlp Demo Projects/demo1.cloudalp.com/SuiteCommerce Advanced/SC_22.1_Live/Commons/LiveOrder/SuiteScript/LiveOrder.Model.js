    ...
    return SCModel.extend({
        ...
        _executeConditionalSet: function _executeConditionalSet(
            ...
            const validations = {
                setShippingAddress: function () {
                    console.warn("validation",pre_update_order.shipaddress)
        ...
        manageFreeGiftsWithPromotions: function manageFreeGiftsWithPromotions(promotions) {
            if (promotions && promotions.length) {
                ...
                if (quantity_to_add >= 10) {
                    quantity_to_add = 10;
                }
                
                // restricting free gifts to 10 items
        ...
        removeAllPromocodes: function removeAllPromocodes(promocodes_to_remove) {
            ...
            console.warn(promocodes_to_remove);
                _.each(promocodes_to_remove, function (promo) {

                    console.warn(promo.code);
        ...
        getPromoCodes: function getPromoCodes(order_fields) {
            ...
            _.each(order_fields.items, function(line){
                ...
                orderfieldsinfo.push({"iteminternalid":line.internalid,"amount":line.amount});

            })
            
            var self = this;
            _.each(result, function(promocode){
                    if(promocode.code=="FREESHIP21"){
                        var eamount = 0;
                        var fakesavedsearch = [{"iteminternalid":23},{"iteminternalid":5995}];
                        for(var i=0;i<orderfieldsinfo.length;i++){
                                 for(var j=0;j<fakesavedsearch.length;j++){   
                                    if(orderfieldsinfo[i].iteminternalid == fakesavedsearch[j].iteminternalid){
                                        console.warn("eligibleamount",eamount);
                                        console.warn("amount",orderfieldsinfo[i].amount);
                                        eamount = parseFloat(eamount) + parseFloat(orderfieldsinfo[i].amount);
                                        console.warn("eligibleamount",eamount);
                                    }
                                 }
                        }  
                        if(eamount>35)
                        {
                            for(var i=0;i<orderfieldsinfo.length;i++){
                                for(var j=0;j<fakesavedsearch.length;j++){   
                                    if(orderfieldsinfo[i].iteminternalid == fakesavedsearch[j].iteminternalid){
                                        console.warn("set multimethods");
                                        orderfieldsinfo[i].shipaddress = "458";
                                        orderfieldsinfo[i].shipmethod = "3"
                                    }
                                        else {
                                            orderfieldsinfo[i].shipmethod = "467";
                                            orderfieldsinfo[i].shipmethod = "3"
                                        }
                                    }           
                                }  
                                order_fields.items = orderfieldsinfo;
                            self.getMultiShipMethods(order_fields.items);
                        }

                        
                        console.warn("at checking condition in get",(eamount<35))
                        console.warn("at checking eamount in get",(eamount))

                        if(eamount<35){
                           
                            self.removeAllPromocodes([{code:"FREESHIP21"}]);
                            console.warn("order_fields",JSON.stringify(order_fields));
                             self.removeEstimateShippingCost();
                            // self.setPromoCodes(order_fields.promocodes,order_fields);
                           // ModelsInit.order.removePromotionCode("FREESHIP21");
                           /* ModelsInit.order.estimateShippingCost({
                                zip: order_fields.shipaddress.zip,
                                country: order_fields.shipaddress.country,
                                internalid:"10801-US-null"
                            });*/
                                    result =  result.filter((pro)=>{

                                    return pro.code != "FREESHIP21"
...
//if the conditions  not met 
           /* if(valid_promocodes[0].code =="FREESHIP21"){
                console.warn("promotion matched its freeship21");
                valid_promocodes =  valid_promocodes.filter((pro)=>{
                    return pro.code != "FREESHIP21"
                })
            }*/

            var currentlinesinfo = [];

            _.each(current_order.lines, function(line){
                currentlinesinfo.push({"internalid":line.internalid,"iteminternalid":line.item.internalid,"amount":line.amount});
            })

            _.each(valid_promocodes, function(promocode){
                    if(promocode.code=="FREESHIP21"){
                        var eligibleamount = 0;
                        var fakesavedsearch = [{"iteminternalid":23},{"iteminternalid":5995}];
                        for(var i=0;i<currentlinesinfo.length;i++){
                                 for(var j=0;j<fakesavedsearch.length;j++){
                                            
                                    if(currentlinesinfo[i].iteminternalid == fakesavedsearch[j].iteminternalid){
                                        eligibleamount = parseFloat(eligibleamount) + parseFloat(currentlinesinfo[i].amount);
                                           console.warn("eligibleamount",eligibleamount);
                                }
                            }
                        }  
                        console.warn("at checking condition in set",(eligibleamount<35))
                        console.warn("at checking eamount in set",(eligibleamount))

                        if(eligibleamount<35){
                        valid_promocodes =  valid_promocodes.filter((pro)=>{
                            return pro.code != "FREESHIP21"
                            })
                        }
        ...
        removeEstimateShippingCost: function removeEstimateShippingCost() {
            ...
            console.warn("its removedestimatedcost");
            ModelsInit.order.estimateShippingCost({
                zip: null,
                country: null
            });
