...
{{#if showSelector}}
    <a class="address-details-selector {{#if isSelected}}address-details-selected{{/if}}" data-action="select" data-id="{{internalid}}">
        <input type="radio" name="address-options-{{manageOption}}" class="address-details-selector-option"  {{#ifEquals this.manageOption "billaddress"}} 
            id="bill-{{internalid}}" {{else}}  id="ship-{{internalid}}" {{/ifEquals}} data-id="{{internalid}}" value="{{internalid}}" {{#if isSelected}} checked {{/if}}>
        {{#if isSelected}}
