		...
		{{#if showHeading}}
			{{#if isUncollapsible}}
				<div class="facets-faceted-navigation-item-facet-group-expander">
					...
					{{#if selectedFacetLength }}
						<small class="factes-facted-navigation-sel-filter"> {{translate '$(0) Selected' selectedFacetLength }}
						</small>
						<a href="{{removeLink}}" class="facets-faceted-navigation-facets-clear">
			...
			{{else}}
				<a href="#" class="facets-faceted-navigation-item-facet-group-expander" data-toggle="collapse"
					data-target="#{{htmlId}}-wrapper" data-type="collapse" title="{{facetDisplayName}}">
					...
					<small class="facte-facted-all"> {{translate 'All' }} </small>
					{{#if showRemoveLink}}
						<a class="facets-faceted-navigation-item-filter-delete" href="{{removeLink}}">
							<i class="facets-faceted-navigation-item-filter-delete-icon"></i>
						</a>
			...
			<div class="facets-faceted-navigation-item-facet-group-content">
					...
					{{#each displayValues}}
						<li>
							<a class="facets-faceted-navigation-item-facet-option {{#if isActive}}option-active{{/if}}"
								...
								{{#if ../isMultiSelect}}
									<input type="checkbox" class="facets-faceted-navigation-item-facet-multi" {{#if isActive}}checked
										{{/if}} />
								{{/if}}
								<span class="facets-nav-display-name">{{displayName}}</span>
				...
				{{#if showExtraValues}}
					...
					<div class="facets-faceted-navigation-item-optionlist-extra-wrapper">
						<a class="facets-faceted-navigation-item-category-optionlist-extra-button" data-toggle="collapse"
							data-target="#{{htmlId}} .facets-faceted-navigation-item-facet-optionlist-extra" data-action="see-more">
