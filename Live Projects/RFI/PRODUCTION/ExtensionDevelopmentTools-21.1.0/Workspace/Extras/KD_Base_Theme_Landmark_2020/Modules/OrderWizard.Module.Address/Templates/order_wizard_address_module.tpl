<div class="order-wizard-address-module">
	...
	<div class="order-wizard-address-module-show-addresses-container">
		...
		{{/if}}
		{{#ifEquals this.manageValue "shipaddress"}}
		<div id="pickup-details" class="custom-control custom-radio" style="padding-bottom: 12px;">
			<label class="custom-control-label" for="defaultGroupExample2">
				Customer Pickup (Seven Hills)
			</label><br>
			<span>Yes </span> <input type="radio" {{#if isPickUp}} checked {{/if}} class="custom-control-input"
				id="defaultGroupExample1" name="groupOfDefaultRadios"><br>
			<span>No </span><input type="radio" {{#unless isPickUp}} checked {{/unless}} class="custom-control-input"
				id="defaultGroupExample2" name="groupOfDefaultRadios"><br>
		</div>
		<div {{#unless isPickUp}} style="display:none" {{/unless}} id="memo-details">
			Pick Up Notes.<br>

			<textarea type="text" id="custbody_kd_notes_from_customer"
				value={{this.model.options.custbody_kd_notes_from_customer}}
	...
	{{log this.model.options.custbody_rfi_c_memo_internal}}
