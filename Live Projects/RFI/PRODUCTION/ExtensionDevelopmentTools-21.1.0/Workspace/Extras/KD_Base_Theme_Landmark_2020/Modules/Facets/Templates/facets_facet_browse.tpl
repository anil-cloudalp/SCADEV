...
<section class="facets-facet-browse">
	...
	{{#if showResults}}
		...
		<header class="facets-facet-browse-header">
			...
			{{#if isCategory}}
				<h2 class="facets-facet-browse-title browse-tit-cat" data-quantity="{{total}}">
					{{#if keywords}}
						{{#if isTotalProductsOne}}
							{{translate '1 Result for <span class="facets-facet-browse-title-alt">$(0)</span>' keywords}}
						{{else}}
			...
			{{else}}
				{{#if keywords}}
				<h1>{{translate 'Results for: <span class="facets-facet-browse-title-alt">$(1)</span>' total keywords}}</h1>
				{{else}}
				<h1 class="maintit-search">{{translate 'Shop'}}</h1>
				{{/if}}
				<h2 class="facets-facet-browse-title browse-tit-search" data-quantity="{{total}}">
					{{#if keywords}}
						{{#if isTotalProductsOne}}
							{{translate '1 Product' keywords}}
						{{else}}
							{{translate '$(0) Products' total keywords}}
						{{/if}}
					{{else}}
						{{#if isTotalProductsOne}}
							{{translate '1 Product'}}
