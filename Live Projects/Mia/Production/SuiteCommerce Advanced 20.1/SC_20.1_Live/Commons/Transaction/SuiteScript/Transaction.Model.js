...
define('Transaction.Model', [
    ...
    return SCModel.extend({
		...
		// SEARCHES
		// @method getPaginatedSearchResultsBackOrders
		...
		getPaginatedSearchResultsBackOrders: function(options){
			options = options || {};
			...
			if (!do_real_count || options.column_count) {
			   var srchRes =  Application.getAllSearchResults(record_type, filters, columns);
				result.totalRecordsFound = srchRes.length;
			}
			if (
				do_real_count ||
		...
		///customization for backorders///
        backOrders: function(data){ 
            ...
            this.columns = {
                ...
                amount: new nlobjSearchColumn('amount'),
                quantity: new nlobjSearchColumn('quantity'),
                quantityshiprecv: new nlobjSearchColumn('quantityshiprecv'),
                formulatext: new nlobjSearchColumn('formulatext')
            }; 
            ...
            if (this.data.page === 'all') {
                const search = nlapiLoadSearch('salesorder', 'customsearch_back_orders_2');
                ...
                switch (this.data.sort) {
                    // sort by date
                    case 'trandate':
                        cols[0].setSort(self.data.order > 0);
                        break;
                    
                    // sort by tranid
                    case 'tranid':
                        cols[1].setSort(self.data.order > 0);
                        break;
                    
                    // sort by amount
                    case 'amount':
                        cols[4].setSort(self.data.order > 0);
                        break;    
                }

                this.search_results = this.getPaginatedSearchResultsBackOrders({
                    record_type: 'transaction',
                    filters: fils,
                    columns:cols,
                    page: this.data.page || 1,
            ...
            const records = _.map(
                ...
                function(record) {
                    ...
                    ////customization for duedate/receive by///
                    
                    var required_file = nlapiLoadFile('34331313');
                    var required_file_contents = required_file.getValue();
                    var required_file_contents_in_obj = JSON.parse(required_file_contents);
                    var req_date_obj = _.find(required_file_contents_in_obj,function(x){
                        var item_name = record.getText('item');
                        return Object.keys(x)[0] == item_name;
                    });
                    
                    if(!_.isUndefined(req_date_obj)){
                        var due_received = Object.keys(req_date_obj);
                        var duedate = req_date_obj[due_received];
                    }
                    /// customization for duedate/receive by///
                    ...
                    const result = {
                        ...
                        status: {
                            // @class Transaction.Status
                            // @property {String} internalid
                            internalid: record.getValue('statusref'),
                            // @property {String} name
                            name: record.getText('statusref')
                        },
                        // @property {item}
                        item_id: {
                            // @class Transaction.Item
                            // @property {String} internalid
                            internalid: record.getValue('item'),
                            // @property {String} name
                            name: record.getText('item')
                        },
                        
                        // @property {duedate/receiveby}
                        duedate: duedate,
                        
                        // @class Transaction.Model.List.Result.Record
                        // @property {Number} amount
                        // amount: Utils.toCurrency(record.getValue(self.amountField)),
                        amount: Utils.toCurrency(record.getValue('amount')),
                        // @property {Currency?} currency
                        currency: self.isMultiCurrency ? // @class Currency
                              {
                                  // @property {String} internalid
                        ...
                        // @property {String} amount_formatted
                        amount_formatted: Utils.formatCurrency(
                            record.getValue('amount'),
                            selected_currency_symbol
                        ),
                        //@property {quantity}
                        quantity: record.getValue('quantity'),
                        //@property {quantitybackordered}
                        quantitybackordered: record.getValue('formulatext'),
                        //@property {quantityshipped}
                        quantityshipped: record.getValue('quantityshiprecv')
                    };
                    return self.mapListResult(result, record);  
                }
            );
