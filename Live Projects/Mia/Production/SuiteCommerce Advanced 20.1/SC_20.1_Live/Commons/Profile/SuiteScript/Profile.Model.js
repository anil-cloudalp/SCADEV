...
define('Profile.Model', ['SC.Model', 'SC.Models.Init', 'PaymentMethod.Model','GetMacDetails.Model', 'Utils','underscore'], function (
    SCModel,
    ModelsInit,
    PaymentMethodModel,
    GetMacDetailsModel,
    ...
    return SCModel.extend({
        ...
        get: function () {
			...
			var custentity9 = profile.customfields ? _.findWhere(profile.customfields,{'name': 'custentity9'}) : '';
			var split_char_5 = String.fromCharCode(5);
			
			if(custentity9){
				var values = (custentity9.value)? custentity9.value.split(split_char_5): [];
				profile.isChannelUcUser = (values.indexOf('9') != -1)? true: false;
			}
			var marketingBanners  = GetMacDetailsModel.getMarketingBanners(profile.priceLevel);
					
			profile.marketingBanners = (marketingBanners && _.has(marketingBanners,'results'))? marketingBanners.results : '';
