...
define('LiveOrder.Model', [
    ...
    return SCModel.extend({
        ...
        get: function get(threedsecure) {
			...
			//checks | updates ship methods on custom rules
			
			if(ModelsInit.session.isLoggedIn2() && Utils.isCheckoutDomain() && !result.ismultishipto && result.shipaddress){
				
				this.verifyShipMethod(result);
			}
			if (
                !threedsecure &&
        ...
        submit: function submit(threedsecure) {
            ...
            if (!threedsecure && payment && payment.creditcard && payment.creditcard.paymentmethod &&
                payment.creditcard.paymentmethod.creditcard === 'T' && this.isThreedSecureEnabled) {
        ...
        getLines: function getLines(order_fields) {
            ...
            const orderedLines = _.sortBy(lines, function(line) {
            ...
            if (this.isCardHolderAuthenticationSupported) {
                ...
                switch (cardHolderAuthenticationRecord.getValue('status')) {
                    ...
                    case 'WAITING_FOR_DEVICE_AUTHENTICATION':
                        const deviceAuthenticationInput = CardHolderAuthentication.searchAuthenticateDeviceInputFields(
                            cardHolderAuthenticationRecord.id
                        );
                        const deviceAuthenticationFormURL = ModelsInit.session.getAbsoluteUrl2(
                            'checkout',
                            '../deviceAuthenticationForm.ssp?action=' +
                                cardHolderAuthenticationRecord.getValue(
                                    'authenticatedeviceformaction'
                                ) +
                                '&form=' +
                                cardHolderAuthenticationRecord.getValue(
                                    'authenticatedeviceformid'
                                ) +
                                '&data=' +
                                deviceAuthenticationInput[0].getValue('value')
                        );
                    ...
                    case 'WAITING_FOR_SHOPPER_CHALLENGE':
                        const challengeShopperInput = CardHolderAuthentication.searchChallengeShopperInputFields(
                            cardHolderAuthenticationRecord.id
                        );
                        const challengeShopperFormURL = ModelsInit.session.getAbsoluteUrl2(
                            'checkout',
                            '../challengeShopperForm.ssp?action=' +
                                cardHolderAuthenticationRecord.getValue(
                                ...
                                '&data=' +
                                challengeShopperInput[0].getValue('value')
                        );
		...
		verifyShipMethod: function(result){
			
			var stateAmt = ['NSW','QLD','VIC','ACT','SA'];
			var quoteBased = ['WA','NT','TAS'];
			const StartrackQuoteBased = '15708';
			const Startrack = '1479';
			const StartrackDropShip = '15709';
			
			const commboxLine = _.find(result.lines, function(line) {
                return line.item.custitem_os_brand == 'Commbox';
            });
			
			//result.shipmethod_org = result.shipmethod;
			
			if(commboxLine){
				result.preffered_shipmethod = StartrackQuoteBased;
				result.shipmethods = _.filter(result.shipmethods, function(method){ return (method.internalid == StartrackQuoteBased); });
			}
			else{
				const dropship = result.options.custbody_is_this_a_dropship;
				
				const addresses = require('Address.Model').list();
				
				const currentAddress = _.find(addresses, function(address) {
					return address.internalid === result.shipaddress;
				});
				
				var stateCode = _.has(currentAddress,'state')?currentAddress.state: '';
				
				
				if(stateCode && quoteBased.indexOf(stateCode) != -1){
					result.preffered_shipmethod = StartrackQuoteBased;
					result.shipmethods = _.filter(result.shipmethods, function(method){ return (method.internalid == StartrackQuoteBased); });
				}
				if(result.preffered_shipmethod != StartrackQuoteBased){
					
					if(dropship && dropship == '1'){
						//for yes only
						result.preffered_shipmethod = StartrackDropShip;
						result.shipmethods = _.filter(result.shipmethods, function(method){ return (method.internalid == StartrackDropShip); });
					}
					if(result.preffered_shipmethod != StartrackDropShip){
						
						if(stateCode && stateAmt.indexOf(stateCode) != -1){
							result.preffered_shipmethod = Startrack;
							result.shipmethods = _.filter(result.shipmethods, function(method){ return (method.internalid == Startrack); });
