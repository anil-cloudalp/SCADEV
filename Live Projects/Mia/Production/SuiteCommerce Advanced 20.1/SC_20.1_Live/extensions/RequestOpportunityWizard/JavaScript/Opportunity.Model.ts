...
/// <amd-module name="Opportunity.Model"/>
...
// @class Opportunity.Model @extends Transaction.Model
export = TransactionModel.extend({
    // @property {String} urlRoot
    urlRoot: 'services/Opportunity.Service.ss?recordtype=opportunity',

    initialize: function(): void {
        TransactionModel.prototype.initialize.apply(this, arguments);

        // metadata of the attribute options(custom fields)
        if (SC.ENVIRONMENT.customFieldsMetadata && SC.ENVIRONMENT.customFieldsMetadata.opportunity) {
            this.__customFieldsMetadata = SC.ENVIRONMENT.customFieldsMetadata.opportunity;
        }
    },

    // @method submit Method invoked when the user a new Opportunity want to be created
    // @return {jQuery.Deferred}
    submit: function() {
        const loggers = Loggers.getLogger();
        const actionId = loggers.start('Request an Opportunity');
    ...
    // the default Opportunity values are preserve
