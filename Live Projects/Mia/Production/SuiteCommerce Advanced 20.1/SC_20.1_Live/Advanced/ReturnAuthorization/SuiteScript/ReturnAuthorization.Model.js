    ...
    return Transaction.extend({
        ...
        getExtraRecordFields: function() {
            ...
            if (this.isSCISIntegrationEnabled && this.result.recordtype === 'creditmemo') {
                ...
                this.result.amountpaid_formatted = Utils.formatCurrency(
                    this.record.getFieldValue('amountpaid'),
                    this.currencySymbol
                );
                this.result.amountremaining = Utils.toCurrency(
                    this.record.getFieldValue('amountremaining')
                );
                this.result.amountremaining_formatted = Utils.formatCurrency(
                    this.record.getFieldValue('amountremaining'),
                    this.currencySymbol
                );
        ...
        isCancelable: function() {
            return (
                this.result.recordtype === 'returnauthorization' &&
        ...
        getApply: function() {
            ...
            for (let i = 1; i <= this.record.getLineItemCount('apply'); i++) {
                if (self.record.getLineItemValue('apply', 'apply', i) === 'T') {
                    const internalid = self.record.getLineItemValue('apply', 'internalid', i);
                    ...
                    self.result.applies[internalid] = {
                        ...
                        amount: Utils.toCurrency(
                            self.record.getLineItemValue('apply', 'amount', i)
                        ),
        ...
        create: function(data) {
            const return_authorization = nlapiTransformRecord(
                data.type,
                data.id,
                'returnauthorization'
            );
            const transaction_lines = this.getTransactionLines(data.id);
        ...
        getTransactionLines: function(transaction_internalid) {
            ...
            const columns = [new nlobjSearchColumn('line'), new nlobjSearchColumn('rate')];
        ...
        getCreatedFrom: function() {
            let created_from_internalid;
            let created_from_name;
            let recordtype;
            let tranid;
            ...
            if (this.isSCISIntegrationEnabled && this.result.recordtype === 'creditmemo') {
                ...
                created_from_internalid = nlapiLookupField(
        ...
        setLines: function(return_authorization, lines, transaction_lines) {
            let line_count = return_authorization.getLineItemCount('item');
            let add_line = true;
            let i = 1;

            while (i <= line_count) {
                const line_item_value = return_authorization.getLineItemValue('item', 'id', i);

                add_line = this.findLine(line_item_value, lines);
                if (add_line) {
                    const transaction_line = _.findWhere(transaction_lines, {
					...
					if(add_line.serialNumbers && add_line.serialNumbers.length > 0){
						
						return_authorization.setLineItemValue(
							'item',
							'custcol_serial_numbers_retruns',
							i,
							add_line.serialNumbers.toString()
						);
					}

					i++;
                } else {
                    const item_type = return_authorization.getLineItemValue('item', 'itemtype', i);
        ...
        setExtraListFilters: function() {
            ...
            if (this.data.createdfrom) {
                ...
                const columns = nlobjSearchColumn('internalid');
                ...
                internal_ids = _(
                    Application.getAllSearchResults('transaction', _.values(filters), columns) || []
                ).pluck('id');
        ...
        setExtraListColumns: function() {
            if (this.data.getLines && this.data.page === 'all') {
                ...
                this.columns.storedisplayname_item = new nlobjSearchColumn(
                    'storedisplayname',
                    'item'
                );
        ...
        mapListResult: function(result, record) {
            if (result && result.currency) {
                const currency = Utils.getCurrencyById(result.currency.internalid);
            ...
            if (this.data.getLines && this.data.page === 'all') {
                ...
                _.each(this.search_results || [], function(record) {
                    if (record.getValue('mainline') !== '*') {
                        const record_id = record.getId();
                        const result = _.findWhere(self.results, { internalid: record_id });

                        if (result) {
                            result.lines = result.lines || [];

                            const item_id = record.getValue('internalid', 'item');
                            const item_type = record.getValue('type', 'item');
                            ...
                            if (item_type !== 'Discount') {
                                result.lines.push({
                                    ...
                                    rate_formatted: Utils.formatCurrency(
                                        record.getValue('rate'),
                                        self.currencySymbol
                                    ),
                                    tax_amount: Utils.toCurrency(
                                        Math.abs(record.getValue('taxtotal'))
                                    ),
                                    tax_amount_formatted: Utils.formatCurrency(
                                        Math.abs(record.getValue('taxtotal')),
                                        self.currencySymbol
                                    ),
                                    amount: Utils.toCurrency(
                                        Math.abs(record.getValue(self.amountField))
                                    ),
                                    amount_formatted: Utils.formatCurrency(
                                        Math.abs(record.getValue(self.amountField)),
                                        self.currencySymbol
                                    ...
                                    item: {
                                        ...
                                        storedisplayname: record.getValue(
                                            'storedisplayname',
                                            'item'
                                        ),
                ...
                _.each(this.results, function(result) {
                    _.each(result.lines, function(line) {
                        const item_details = self.store_item.get(
                ...
                Application.getAllSearchResults('transaction', _.values(filters), columns).forEach(
                    function(record) {
                        quantities[record.getValue('internalid', null, 'group')] = record.getValue(
                            'quantity',
                            null,
                            'sum'
                        );
        ...
        postGet: function() {
            ...
            const self = this;
            const isCreditMemo = this.result.recordtype === 'creditmemo';
            let record_type = '';
            let created_from_internalid = '';
            let created_from_name = '';
            let cretaed_from_tranid = '';
            ...
            Application.getAllSearchResults('transaction', _.values(filters), columns).forEach(
                function(record) {
                    ...
                    if (isCreditMemo) {
                        const line =
                            self.result.lines[
                                self.result.internalid + '_' + record.getValue('line')
                            ];
