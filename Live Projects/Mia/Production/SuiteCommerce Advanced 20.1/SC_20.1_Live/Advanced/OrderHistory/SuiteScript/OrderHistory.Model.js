...
define('OrderHistory.Model', [
    ...
    return Transaction.extend({
        ...
        setExtraListFilters: function() {
			//for adding PO filter
				if (this.data.po) {
					this.filters.po_operator = 'and';
					this.filters.po = ['formulatext: {otherrefnum}','contains',this.data.po];
				}
			...
			if (this.data.filter === 'status:billed') {
        ...
        getFulfillments: function() {
            ...
            Application.getAllSearchResults('salesorder', filters, columns).forEach(function(
				...
				const quantity_fulfilled_org = new BigNumber(ffline.getValue('quantity','fulfillingtransaction'));
                ...
                if (fulfillment_id) {
                    ...
                    self.result.fulfillments[fulfillment_id] = self.result.fulfillments[
                        ...
                        date: ffline.getValue('trandate','fulfillingtransaction'),
                    ...
                    self.result.fulfillments[fulfillment_id].lines.push({
                        internalid: line_internalid,
                        quantity: quantity_fulfilled_org.toNumber()
                    });
                }

                if (line) {
                    const conversion_units =
                        is_uom_enabled && quantity.gt(zero) && quantity_uom.gt(zero)
