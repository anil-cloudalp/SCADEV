...
<div class="facets-faceted-navigation-item-facet-group" id="{{htmlId}}" data-type="rendered-facet" data-facet-id="{{facetId}}">
	...
	{{else}}
		<a href="#" class="facets-faceted-navigation-item-facet-group-expander"  data-toggle="customdropdown" data-target="#{{htmlId}}-category-wrapper" data-type="collapse" title="{{facetDisplayName}}">
        <i class="facets-faceted-navigation-item-facet-group-expander-icon rotate"></i>
        <h4 class="facets-faceted-navigation-item-facet-group-title">{{facetDisplayName}} <span class="filternumber"></span></h4>
		{{#if showRemoveLink}}
			<a class="facets-faceted-navigation-item-filter-delete" href="{{removeLink}}">
				<i class="facets-faceted-navigation-item-filter-delete-icon"></i>
			</a>
	...
	{{/if}}
	{{/if}}
	<div class="{{#if isCollapsed}} collapse {{else}} collapse in {{/if}} facets-faceted-navigation-item-facet-group-wrapper beforeContent-text" id="{{htmlId}}-category-wrapper">
			...
			<ul class="facets-faceted-navigation-item-facet-optionlist">
				...
				<li>
					<a class="facets-faceted-navigation-item-facet-option {{#if isActive}}option-active{{/if}}" href="{{link}}" title="{{label}}">
						{{#if ../isMultiSelect}}
							<input type="checkbox"  class="facets-faceted-navigation-item-facet-multi {{#if displayName}} filtercheckbox {{/if}}" value="{{displayName}}" {{#if isActive}}checked {{/if}} />
						{{/if}}
