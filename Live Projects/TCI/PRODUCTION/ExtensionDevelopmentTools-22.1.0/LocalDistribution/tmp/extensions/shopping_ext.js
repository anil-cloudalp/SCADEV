				...
				trackYourOrder.fetch({
					...
					if (results.success) {
						...
						_.each(results.data, function(item) {
							if (item.status === 'Deposited' || item.status === 'Paid In Full') {
    ...
    saveForm: function () {
      ...
      if (itemsToReturn.length) {
        returns.fetch({
          ...
          ,	autohide: true
          , callBack: function(){ 
              Backbone.history.navigate('request-a-return', {
                  trigger: true
              }); 
            }
          });
          self.parentView.options.application.getLayout().showInModal(returnConfirmationView);
        });
      }
    },
		getContext: function () {
