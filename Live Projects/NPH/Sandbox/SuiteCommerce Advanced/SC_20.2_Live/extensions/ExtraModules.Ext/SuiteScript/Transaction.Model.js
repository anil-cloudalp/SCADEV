...
define('Transaction.Model', [
    ...
    return SCModel.extend({
        ...
        list: function(data) {
			...
			if(this.name == 'OrderHistory')
			{	
				var validCustomers = {};
				var entityArr = [];
				var custid = this.data.filter;
				
					var suiteletUrl = nlapiResolveURL('Suitelet', '488','customdeploy_appf_get_customers_web','external');
					
					suiteletUrl += '&custid=' + nlapiGetUser();
					...
					if(response.getCode() == 200){
						...
						validCustomers = response.getBody();
						validCustomers = JSON.parse(validCustomers);
					}
					if(!_.isEmpty(validCustomers)){
						if(validCustomers.hasOwnProperty('listOptions')){
							listOptionsArr = validCustomers.listOptions;
						}
						if(validCustomers.hasOwnProperty('entityIDs')){
							entityArr = validCustomers.entityIDs;
						}
					}

				//nlapiLogExecution('DEBUG','validCustomers:',JSON.stringify(validCustomers));
				
				if(custid == '' || custid == null){
					custid = nlapiGetUser();
				}
				if(custid && custid != null && custid != 'ALL'){
					this.filters = {
						'entity': ['entity', 'is', custid]
					};
				}
            ...
            const records = _.map(
                ...
                function(record) {
                    ...
                    const result = {
						...
						entity:(self.name == 'OrderHistory')?{internalid:record.getValue('entity'),name:record.getText('entity')}:{},
            ...
            if (this.data.page === 'all') {
				...
				if(this.name == 'OrderHistory' && listOptionsArr.length > 0)
					this.results.listOptions = listOptionsArr;
