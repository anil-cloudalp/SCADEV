    ...
    return SCModel.extend({
		...
		getAllitemids: function(fieldoptions)
		{
			...
			{	
				...
				{
					var psalmNumStr = fieldoptions.PsalmNumberinternalid;
					var psalmNumbers = psalmNumStr.split(",");
					if(psalmNumbers.length > 0)
					{
						var psalmFormulaeText = "CASE WHEN {custitem_appf_mr_psalm_number} like '%";
						for(var p=0; p < psalmNumbers.length; p++)
						{
							psalmFormulaeText += psalmNumbers[p];
							if(p+1 < psalmNumbers.length)
								psalmFormulaeText += "%' THEN 'Y' WHEN {custitem_appf_mr_psalm_number} like '%";
							else
								psalmFormulaeText += "%' THEN 'Y'  ELSE 'N' END";							
						}
						if(fieldoptions.HymnNumberinternalid != null && fieldoptions.HymnNumberinternalid != '')
						{
							fils[1] = new nlobjSearchFilter('formulatext',null,'is','Y');
							fils[1].setFormula(psalmFormulaeText);
						}
						else
						{
							fils[0] = new nlobjSearchFilter('formulatext',null,'is','Y');
							fils[0].setFormula(psalmFormulaeText);
						}
					}
