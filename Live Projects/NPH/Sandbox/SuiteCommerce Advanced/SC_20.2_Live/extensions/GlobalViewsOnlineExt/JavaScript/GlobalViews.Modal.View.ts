...
const GlobalViewsModalView: any = BackboneView.extend({
    ...
    getContext: function() {
        ...
        return {
			...
			pageHeader: (this.options.childViewIstance.attributes)?(this.options.childViewIstance.attributes.id == 'Get_info'? this.options.childViewIstance.id || '': this.options.pageHeader || ''):this.options.pageHeader || '',
			...
			musicresource:(this.options.childViewIstance.attributes.id == 'Get_info')?true:false
        };
    }
});
