...
export = BackboneView.extend({
    ...
    events: {
		...
		'click #price-filter-btn': 'updateRangeSelection'
    },
    ...
    updateRangeSelection: function(e, slider) {
		...
		var minval:any = parseFloat(String($('#price-filter-min').val()));
		var maxval:any = parseFloat(String($('#price-filter-max').val()));
			
		
        const { translator } = this.options;
        // the currently selected slider values
        const slider_values = _.isUndefined(slider)? {low: minval, high: maxval} : slider.values;
