    ...
    const CMSAdapterComponent = SCBaseComponent.extend({
        ...
        updateContent: function updateContent(
            ...
            if (rawCCTs) {
                ...
                const self = this;
                const cct_container = customContentTypesViews[cctInstanceId];
                const async_operations = [];

                if (cct_container) {
                    const container = customContentTypesViews[cctInstanceId];
                    const placeholder = SCBaseComponentChildViewsComponent.getPlaceholderViews(
                        `[data-cms-area="${container}"]`
                    );
                    const views = placeholder && placeholder.views;

                    _.each(views, (view: any) => {
                        const cctContainer = view.getChildViewInstance(container, cctInstanceId);

                        if (cctContainer) {
                            async_operations.push(this._updateSettings(cctContainer, cctSettings));
                        }
                    });
                ...
                if (views) {
                    _.each(views, function(view: any) {
                        async_operations.push(
                            view.removeChildViewInstance(container, cctInstanceId, destroyCall)
                        );
                    });
