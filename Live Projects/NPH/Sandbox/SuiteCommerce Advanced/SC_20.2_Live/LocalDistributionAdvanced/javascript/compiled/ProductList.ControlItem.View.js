...
*/
define("ProductList.ControlItem.View", ["require", "exports", "underscore", "product_list_control_item.tpl", "jQuery", "Backbone.View"], function (require, exports, _, product_list_control_item_tpl, jQuery, BackboneView) {
    "use strict";
    return BackboneView.extend({
        ...
        addRemoveProduct: function ($checkbox) {
            ...
            if ($checkbox.is(':checked')) {
                // add to list
                if (_.has(this.line.get('options'), 'models'))
                    _.each(this.line.get('options').models, function (opt) {
                        var optVal = !_.isUndefined(opt.get('value')) ? opt.get('value').internalid : '';
                        if (optVal) {
                            opt.set('value', '');
                        }
                    });
