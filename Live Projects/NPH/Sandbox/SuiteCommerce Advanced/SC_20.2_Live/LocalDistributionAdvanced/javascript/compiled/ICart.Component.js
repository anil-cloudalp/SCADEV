...
*/
define("ICart.Component", ["require", "exports", "underscore", "SC.VisualComponent", "Utils"], function (require, exports, _, SCVisualComponent, Utils) {
    "use strict";
    ...
    // @public @extlayer
    var icart_component = _.extend({}, SCVisualComponent, {
        componentName: 'Cart',
        ...
        _validateCustomer: function _validateCustomer(data) {
            ...
            if (!data || !_.isObject(data.customer)) {
                return this._reportError('INVALID_PARAM', 'Invalid parameter. It must be a valid objec, must contain a customer object and customer must contain a customer_id string');
            }
            // TODO call _reportError (code, description) when an invalid line is passed
        },
        _validatePromocode: function _validatePromocode(promocode) {
            if (!promocode) {
                return this._reportError('INVALID_PARAM', 'Invalid parameter. It must be a valid promocode, must contain a string.');
            }
        },
        _validateInternalid: function _validateInternalid(internalid) {
            if (!internalid) {
                return this._reportError('INVALID_PARAM', 'Invalid parameter. It must be a valid internalid, must contain a string.');
            }
        },
        ...
        _normalizeLine: function _normalizeLine(src_line) {
            var line = Utils.deepCopy(src_line);
            var commonLineAttrs = [
                'internalid',
                'item',
                'quantity',
        ...
        _normalizeSummary: function _normalizeSummary(inner_summary) {
            var summary = Utils.deepCopy(inner_summary);
        ...
        _normalizePromocode: function _normalizePromocode(promocode) {
            var promotion = Utils.deepCopy(promocode);
        ...
        _normalizeShipMethod: function _normalizeShipMethod(inner_ship_method) {
            var ship_method = Utils.deepCopy(inner_ship_method);
        ...
        _normalizeConfirmation: function _normalizeConfirmation(inner_confirmation) {
            var confirmation = Utils.deepCopy(inner_confirmation);
            var commonConfirmationAttrs = [
                'internalid',
                'confirmationnumber',
                'lines',
                'paymentmethods',
        ...
        _normalizePaymentMethod: function _normalizePaymentMethod(inner_payment_method) {
            ...
            var payment_method = Utils.deepCopy(inner_payment_method);
            ...
            var commonGiftCertificateAttrs = ['internalid', 'name', 'creditcard', 'ispaypal', 'key'];
        ...
        _normalizeCustomer: function _normalizeCustomer(inner_customer) {
            var customer = Utils.deepCopy(inner_customer);
            var commonCustomerAttrs = ['internalid', 'name'];
        ...
        _normalizeGlobalDiscount: function _normalizeGlobalDiscount(discount) {
            var item = Utils.deepCopy(discount);
            var commonDiscountAttrs = ['id', 'rate', 'description'];
            return format(item, commonDiscountAttrs);
        },
        _normalizeLineDiscount: function _normalizeLineDiscount(discount) {
            var item = Utils.deepCopy(discount);
            var commonDiscountAttrs = ['discount', 'item'];
            return format(item, commonDiscountAttrs);
        },
        _normalizeGlobalPromotion: function _normalizeGlobalPromotion(promotion) {
            var item = Utils.deepCopy(_.last(promotion));
            var commonPromoAttrs = ['internalid', 'couponcode', 'toApply'];
        ...
        _normalizeAddGlobalPromotionAfter: function _normalizeAddGlobalPromotionAfter(promotions) {
            var promotion = Utils.deepCopy(_.last(promotions));
            var commonPromoAttrs = ['internalid', 'couponcode', 'discountrate'];
        ...
        _normalizeReturnLine: function _normalizeReturnLine(src_line) {
            var returned_line = Utils.deepCopy(src_line);
            var commonReturnLineAttrs = [
                'line',
                'item',
                'quantity',
                'returnedQuantity',
        ...
        _normalizeAddGiftCard: function _normalizeAddGiftCard(src) {
            var gift_card = Utils.deepCopy(src);
            var commonAttrs = ['cert_number', 'amount'];
            return {
...
//# sourceMappingURL=ICart.Component.js.map
