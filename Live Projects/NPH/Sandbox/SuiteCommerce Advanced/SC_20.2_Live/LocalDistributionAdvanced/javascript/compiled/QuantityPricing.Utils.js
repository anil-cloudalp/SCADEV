...
*/
define("QuantityPricing.Utils", ["require", "exports", "underscore"], function (require, exports, _) {
    "use strict";
    // @class QuantityPricing.Utils
    var QuantityPricingUtils = {
        ...
        rearrangeQuantitySchedule: function rearrangeQuantitySchedule(item, children_selection) {
            ...
            var price_schedule_exists_in_matrix_parent = item &&
            ...
            if (price_schedule_exists_in_matrix_parent || price_schedule_exists_in_matrix_child) {
                ...
                var result_quantity_pricing = [];
                if (children_selection && children_selection.length === 1) {
                    var first_children_section = _.first(children_selection);
                    ...
                    var reference_quantity_pricing_1 = _.first(children_quantity_pricing);
                    var self_1 = this;
                    var all_children_are_equal = _.every(children_quantity_pricing, function (child_quantity_pricing) {
                        return self_1.areQuantityPricingEqual(reference_quantity_pricing_1, child_quantity_pricing);
                    });
                    result_quantity_pricing = all_children_are_equal
                        ? self_1.computeRanges(children_quantity_pricing)
                        : [];
                }
                else {
                    result_quantity_pricing = parent_quantity_schedule;
                }
                var quantityPricingTiersAmount_1 = result_quantity_pricing.length;
                if (quantityPricingTiersAmount_1 > 0) {
                    var basePrice = result_quantity_pricing[0].price;
                    result_quantity_pricing = _.map(result_quantity_pricing, function (price_range, index) {
                        return {
                            maximumquantity: item &&
                                item.get('maximumquantity') &&
                                (quantityPricingTiersAmount_1 - 1 === index &&
                            ...
                            discount: (((basePrice - price_range.price) / basePrice) * 100).toFixed(0),
                            notShowQuantityRange: price_range.maximumquantity &&
        ...
        computeRanges: function computeRanges(quantity_pricings) {
            ...
            _.each(grouped_ranges, function (ranges) {
                ...
                var min = _.min(_.pluck(ranges, 'price'));
                var is_range = min !== max;
        ...
        areQuantityPricingEqual: function areQuantityPricingEqual(quantity_pricing_original, quantity_pricing_to_compare) {
            if (quantity_pricing_to_compare &&
                ...
                for (var i = 0; i < quantity_pricing_original.length; i++) {
                    if (quantity_pricing_original[i].maximumquantity !==
    ...
    return QuantityPricingUtils;
});

//# sourceMappingURL=QuantityPricing.Utils.js.map
