...
*/
define("UnitTestHelper", ["require", "exports", "underscore", "jQuery", "Configuration", "Backbone", "jasmine2-typechecking-matchers", "Backbone.View", "Backbone.View.render", "Backbone.Model", "Backbone.View.Plugins", "Utils"], function (require, exports, _, jQuery, Configuration_1, Backbone) {
    "use strict";
    return (function () {
        /*
        ...
        */
        var TestHelper = function (options) {
            ...
            _(Configuration_1.Configuration).extend(this.options.applicationConfiguration || {});
            if (!Configuration_1.Configuration.siteSettings) {
                Configuration_1.Configuration.siteSettings = SC.ENVIRONMENT.siteSettings;
            }
            this.application.Configuration = Configuration_1.Configuration;
            if (this.options.startApplication) {
                var modules = this.options.mountModules && _.isArray(this.options.mountModules)
        ...
        TestHelper.prototype.testViewSelectors = function (view, asserts, data, test_description) {
            describe(test_description || 'Model ID: ' + (data.internalid || data.id), function () {
                _.each(asserts, function (assert) {
                    ...
                    var is_not = assert.operation && assert.operation.indexOf('not.') === 0;
                    var operation = (assert.operation && assert.operation.replace('not.', '')) || 'toBe';
                    var description = _.isFunction(assert.actual)
        ...
        */
        TestHelper.prototype.testModelValidations = function (model, test, test_description) {
            ...
            describe(test_description ||
                'Model ID: ' +
                    (model.internalid || model.id || model.get('id') || model.get('internalid')), function () {
