...
*/
define("NavigationHelper.Plugins.DataTouchPoint", ["require", "exports", "underscore", "Utils", "Configuration", "UrlHelper", "Tracker", "Backbone", "Session", "UrlHelper"], function (require, exports, _, Utils, Configuration_1, UrlHelper_1, Tracker, Backbone) {
    "use strict";
    ...
    var dataTouchPointPlugins = {
        ...
        getLocaleHost: function (locale) {
            ...
            if (available_hosts && available_hosts.length) {
                for (var i = 0; i < available_hosts.length; i++) {
                    ...
                    var lang = _(host.languages).findWhere({ locale: locale });
                    if (lang && lang.host) {
        ...
        preserveLanguage: function (context) {
            ...
            if (Utils.isShoppingDomain()) {
                var locale_host_1 = dataTouchPointPlugins.getLocaleHost(SC.ENVIRONMENT.currentLanguage.locale);
                if (locale_host_1) {
                    url = url.replace(/(http:\/\/)([^/?#]*)([^>]*)/gi, function (all, protocol, host, rest) {
                        return protocol + locale_host_1 + rest;
                    });
                }
                if (/\?$/.test(url)) {
        ...
        generateUrl: function (context, application, layout) {
            if (context.original_touchpoint === application.getConfig().currentTouchpoint) {
                ...
                context.url = Utils.addParamsToUrl(context.clean_hashtag, Utils.parseUrlOptions(context.target_touchpoint));
            }
            else {
                context.url = UrlHelper_1.UrlHelper.fixUrl(context.target_touchpoint);
        ...
        setHashtag: function (layout, application, e) {
            var context = layout.generateNavigationContext(e);
            var target_is_blank = layout.isTargetBlank(e) || (e.button && e.button !== 0);
            var storeLocatorTouchpoint = Configuration_1.Configuration.get('siteSettings.isHttpsSupported')
                ? 'home'
                : 'checkout';
            var isStoreLocator = context.target_data.touchpoint === 'storelocator';
            if (context.target_data.touchpoint) {
                var currentTouchpoint = application.getConfig().currentTouchpoint;
                if (!target_is_blank &&
                    context.target_data.hashtag &&
                    currentTouchpoint &&
                    currentTouchpoint ===
                        (isStoreLocator ? storeLocatorTouchpoint : context.target_data.touchpoint)) {
                    var new_url = Utils.correctURL(context.target_data.hashtag);
                    layout.setUrl(context.$target, new_url);
                }
...
//# sourceMappingURL=NavigationHelper.Plugins.DataTouchPoint.js.map
