...
*/
define("Transaction.Line.Views.Option.View", ["require", "exports", "underscore", "Configuration", "Backbone.View"], function (require, exports, _, Configuration_1, BackboneView) {
    "use strict";
        ...
        getContext: function () {
            ...
            var values = _.map(this.model.get('values') || [], function (value) {
                ...
                if (self.model.get('colors')) {
                    color =
                        self.model.get('colors')[value.label] ||
            ...
            var showOption = true;
            if (this.model.get('cartOptionId') == "custcol_appf_imprint_item_details") {
                if (selected_value.label.indexOf('|') != -1 && selected_value.label.split('|').length > 5)
                    selected_value.label = selected_value.label.split('|')[6];
            }
            if (this.model.get('cartOptionId') == "custcol_appf_imprint_cross" || this.model.get('cartOptionId') == "custcol_appf_holy_bible")
                selected_value.label = this.options.additionalOpt;
            if ((this.options.isCrossHoly && this.model.get('cartOptionId') == "custcol_appf_holy_bible") || selected_value.label == '') {
                showOption = false;
            }
            if (this.model.get('cartOptionId') == 'GIFTCERTRECIPIENTEMAIL') {
                showOption = false;
            }
