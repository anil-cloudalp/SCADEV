...
define("Facets.FacetedNavigationItem.View", ["require", "exports", "underscore", "facets_faceted_navigation_item.tpl", "Utils", "jQuery", "Configuration", "Backbone.View", "Backbone", "Bootstrap.Slider"], function (require, exports, _, facets_faceted_navigation_item_tpl, Utils, jQuery, Configuration_1, BackboneView, Backbone) {
    ...
    return BackboneView.extend({
        ...
        updateRangeSelection: function (e, slider) {
            var minval = parseFloat(String($('#price-filter-min').val()));
            var maxval = parseFloat(String($('#price-filter-max').val()));
            var translator = this.options.translator;
            // the currently selected slider values
            var slider_values = _.isUndefined(slider) ? { low: minval, high: maxval } : slider.values;
