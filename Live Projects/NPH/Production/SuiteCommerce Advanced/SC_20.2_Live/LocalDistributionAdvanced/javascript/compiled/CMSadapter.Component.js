        ...
        var CMSAdapterComponent = SC_BaseComponent_1.SCBaseComponent.extend({
            ...
            addContents: function addContents() {
                ...
                _.each(ccts, function (cct) {
                    ...
                    if (index >= 0 &&
                        customContentTypesViews[cct.instance_id] === cct.selector['data-cms-area']) {
            ...
            addContent: function addContent(cctId, cctInstanceId, cctSelector, cctSettings, renderSettings) {
                if (rawCCTs) {
                    ...
                    if (!customContentTypesViews[cctInstanceId]) {
                        customContentTypesViews[cctInstanceId] = cctSelector['data-cms-area'];
            ...
            updateContent: function updateContent(cctId, cctInstanceId, cctSelector, cctSettings, renderSettings) {
                ...
                if (rawCCTs) {
                    ...
                    var self_1 = this;
                    var cct_container = customContentTypesViews[cctInstanceId];
                    var async_operations_1 = [];
                    if (cct_container) {
                        var container_1 = customContentTypesViews[cctInstanceId];
                        var placeholder = SC_BaseComponent_ChildViewsComponent_1.SCBaseComponentChildViewsComponent.getPlaceholderViews("[data-cms-area=\"" + container_1 + "\"]");
                        var views = placeholder && placeholder.views;
                        _.each(views, function (view) {
                            var cctContainer = view.getChildViewInstance(container_1, cctInstanceId);
                            if (cctContainer) {
                                async_operations_1.push(_this._updateSettings(cctContainer, cctSettings));
                            }
                        });
            ...
            removeContent: function removeContent(cctInstanceId, destroyCall) {
                if (rawCCTs) {
                    ...
                    var container_2 = customContentTypesViews[cctInstanceId];
                    var placeholder = SC_BaseComponent_ChildViewsComponent_1.SCBaseComponentChildViewsComponent.getPlaceholderViews("[data-cms-area=\"" + container_2 + "\"]");
                    var views = placeholder && placeholder.views;
                    var async_operations_2 = [];
                    if (views) {
                        _.each(views, function (view) {
                            async_operations_2.push(view.removeChildViewInstance(container_2, cctInstanceId, destroyCall));
                        });
                        if (destroyCall) {
                            delete customContentTypesViews[cctInstanceId];
                        }
                        return jQuery.when.apply(jQuery, async_operations_2);
                    }
                    return jQuery
                        .Deferred()
                        .reject(this._createError('ERR_INVALID_INSTANCE_ID', Utils.translate("The specified instance id is not registered. Instance Id: " + cctInstanceId)));
