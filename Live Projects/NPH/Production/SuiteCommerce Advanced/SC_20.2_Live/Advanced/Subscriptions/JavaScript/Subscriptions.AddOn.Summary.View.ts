...
const SubscriptionsAddOnSummaryView: any = BackboneView.extend({
    ...
    getInitialPrice: function(): object {
        ...
        const discount = new BigNumber((100 - this.model.get('discount')) / 100).toFixed(precision);
        const initialPrice = new BigNumber(this.model.get('recurringAmount') / discount).toFixed(
            precision
        );
        const discounted = new BigNumber(initialPrice - this.model.get('recurringAmount')).toFixed(
            precision
        );
