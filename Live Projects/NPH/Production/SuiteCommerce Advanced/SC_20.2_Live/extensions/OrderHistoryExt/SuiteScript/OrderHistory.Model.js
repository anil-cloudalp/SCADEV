    ...
    return Transaction.extend({
        ...
        getFulfillments: function () {
            ...
            const columns = [
				...
				new nlobjSearchColumn('custbody_appf_tracking_numbers','fulfillingtransaction'),
            ...
            Application.getAllSearchResults('salesorder', filters, columns).forEach(function (
                ...
                if (fulfillment_id) {
                    ...
                    self.result.fulfillments[fulfillment_id] = self.result.fulfillments[
						...
						trackingLink: ffline.getValue('custbody_appf_tracking_numbers','fulfillingtransaction'),
		...
		downItems: function(data){
			var catid = Configuration.get("Musicianresourcepage.musicianresourceid") || null;
			var filters = {
					'custrecord_appf_customer': ['custrecord_appf_customer', 'is', nlapiGetUser()]
				,	'isinactive_operator': 'and'
				,	'isinactive': ['isinactive', 'is', 'F']
				,	'custrecord_appf_downloadable_item_operator': 'and'
				,	'custrecord_appf_downloadable_item': ['custrecord_appf_downloadable_item.commercecategoryid', 'noneof', catid]
			};

			var columns = [
					new nlobjSearchColumn('itemid','custrecord_appf_downloadable_item')
				,	new nlobjSearchColumn('custrecord_appf_downloadable_item')
				,	new nlobjSearchColumn('custrecord_appf_allow_for_download')
				,	new nlobjSearchColumn('created')
				,	new nlobjSearchColumn('custrecord_appf_so_link')
			...
			{
				filters.date_from_operator = 'and';
			...
			if (data.sort)
			{
				...
				switch (data.sort) {
                // sort by name
                case 'custrecord_appf_downloadable_item':
                    columns[0].setSort(data.order > 0);
                    break;

                // sort by price
                case 'price':
                    columns[4].setSort(data.order > 0);
                    break;
			...
			var search_results = Application.getPaginatedSearchResults({
				record_type: 'customrecord_appf_download_history_web'
			...
			var records = _.map(search_results.records || [], function (record)
			{
				var result = {
					...
					item: record.getValue('itemid','custrecord_appf_downloadable_item')
				,	itemId: record.getValue('custrecord_appf_downloadable_item')
				,	isDownloadable: record.getValue('custrecord_appf_allow_for_download')
				,	created: (record.getValue('created')).split(' ')[0]
				,	tranid: record.getValue('custrecord_appf_so_link')
				,	soName: record.getText('custrecord_appf_so_link')
		...
		musicItems: function(data){
			var filters = [];
					filters.push(new nlobjSearchFilter('custrecord_appf_customer', null,'is', nlapiGetUser()));
					filters.push(new nlobjSearchFilter('isinactive', null,'is', 'F'));
					if(data.catid)
					{	
						filters.push(new nlobjSearchFilter('commercecategoryid','custrecord_appf_downloadable_item','anyof',data.catid));
					}
			...
			if (data.searchParam)
			{				
                filters.push(new nlobjSearchFilter('custitem_appf_musician_resource_search', 'custrecord_appf_downloadable_item', 'contains', data.searchParam));
			}
			if (data.from && data.to)
			{
				data.from = data.from.split('-');
				data.to = data.to.split('-');
				filters.push(new nlobjSearchFilter('created', null, 'within'
								,	new Date(
										data.from[0]
									,	data.from[1]-1
									,	data.from[2]
