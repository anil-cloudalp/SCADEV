...
import CheckoutAsGuestView = require('../../../Advanced/LoginRegister/JavaScript/LoginRegister.CheckoutAsGuest.View');
...
const LoginRegisterView: any = BackboneView.extend({
    ...
    initialize: function(options) {
		...
		var digitalItems = cart.get('lines').filter(function(line:any){
			
			var item = line.getItem();
			return (item.get('custitem_appf_is_downloadable_item') || item.get('custitem10'));

		});
		this.hasDigitalItems = (digitalItems.length > 0);
        ...
        this.enableCheckoutAsGuest =
            ...
            (Configuration.getRegistrationType() === 'optional' ||
                Configuration.getRegistrationType() === 'disabled') && !this.hasDigitalItems;
