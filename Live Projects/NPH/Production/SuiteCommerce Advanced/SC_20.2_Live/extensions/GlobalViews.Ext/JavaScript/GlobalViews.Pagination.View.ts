...
export const GlobalViewsPaginationView: any = BackboneView.extend({
    ...
    getContext: function() {
		...
		for(var i = 0; i< this.pages.length; i++){           
            if(this.parentView.translator && this.parentView.translator.facets[0] && this.parentView.translator.facets[0].config.id == "musiciansresource")
			{
				var url1 = this.pages[i].URL.replace("musiciansresource/", "musiciansresource");
                this.pages[i].URL = url1;
            } 
			else 
			{
                this.pages[i].URL = this.pages[i].URL;                
            }
        }
        // @class GlobalViews.Pagination.View
        return {
            ...
            currentPageURL: this.parentView.translator && this.parentView.translator.facets[0] && this.parentView.translator.facets[0].config.id == "musiciansresource"? (UrlHelper.fixUrl(this.pager(this.current_page))).replace("musiciansresource/", "musiciansresource"):UrlHelper.fixUrl(this.pager(this.current_page)),
            // @property {String} previousPageURL
            previousPageURL: this.parentView.translator && this.parentView.translator.facets[0] && this.parentView.translator.facets[0].config.id == "musiciansresource"? (UrlHelper.fixUrl(this.pager(this.current_page - 1))).replace("musiciansresource/", "musiciansresource"):UrlHelper.fixUrl(this.pager(this.current_page - 1)),
            // @property {String} nextPageURL
            nextPageURL: this.parentView.translator && this.parentView.translator.facets[0] && this.parentView.translator.facets[0].config.id == "musiciansresource"? (UrlHelper.fixUrl(this.pager(this.current_page + 1))).replace("musiciansresource/", "musiciansresource"):UrlHelper.fixUrl(this.pager(this.current_page + 1)),
