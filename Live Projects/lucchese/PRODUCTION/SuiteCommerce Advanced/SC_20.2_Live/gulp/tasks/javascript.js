...
let jsModulesToExport = [];
const tplModulesToExport = {};
...
function generateStarterLocalFiles(config) {
    ...
    const onFile = function(file) {
        ...
        const modulePath = normalized_path.replace(/\\/g, '/').replace(/\.js$/, '');
        if (paths[moduleName]) {
            throw new Error(
    ...
    const onEnd = function() {
        ...
        if (package_manager.isGulpLocal) {
            ...
            const invertedAmdConfigPaths = _.invert(config.amdConfig.paths);
            paths = _.mapObject(paths, function(val) {
                ...
                if (val.indexOf(base_path) === 0) {
                    ...
                    jsModulesToExport.push(invertedAmdConfigPaths[basename] || basename);
                } else if (val.indexOf(thirdparties_path) !== -1 || val.indexOf('javascript-dependencies') !== -1) {
                    jsModulesToExport.push(invertedAmdConfigPaths[basename] || basename);
                    val = val.replace(`${thirdparties_path}/`, '');
                } else {
                    const appName = path.basename(config.exportFile, '.js');
                    tplModulesToExport[appName] = tplModulesToExport[appName] || [];
                    tplModulesToExport[appName].push(basename);
                }
                return val;
            });
        }
...
module.exports = {
    generateStarterLocalFiles
};

function generateEntryPointContent(config) {
    const entry_point = [config.entryPoint];
    const loadTemplateSaveFile = package_manager
        .getGlobsForModule('SC.Extensions', 'javascript-almond-fix')
        .find(filePath => filePath.indexOf('LoadTemplateSafe.js') !== -1);
    const loadTemplateSave = fs.readFileSync(loadTemplateSaveFile);
    const template = _.template(
        `try {\n${loadTemplateSave}\nrequire.config(<%= require_config %>);\nvar ${entry_point[0].replace(
            /\./g,
            ''
        )}= true;\n }\ncatch(e){};`
    );
...
function compileSCA(config, cb, generateTemplateManifests) {
    ...
    config.amdConfig.onModuleBundleComplete = function(data) {
        if (generateTemplateManifests) {
            const manifestName = `templates-manifest-${path.basename(
                config.exportFile,
                '.js'
            )}.json`;
    ...
    config.amdConfig.onBuildWrite = function(moduleName, pathName, contents) {
        const isTpl = path.extname(moduleName) === '.tpl';
        const appName = path.basename(config.exportFile, '.js');
        if (isTpl) {
            tplModulesToExport[appName] = tplModulesToExport[appName] || [];
            tplModulesToExport[appName].push(moduleName);
        } else {
            jsModulesToExport.push(moduleName);
        }
    ...
    config.amdConfig.generateSourceMaps = false;
    ...
    if (!package_manager.isGulpLocal) {
        stream = stream.pipe(amdOptimize(config.amdConfig));
    }
    ...
    stream = stream
        .pipe(
            map(function(file, done) {
                const exposedModules = `SC.ENVIRONMENT.JS_MODULE_NAMES = ${JSON.stringify(
                    jsModulesToExport
                )};`;
                const extLayPatch = fs.readFileSync(
                ...
                let extraContent = `\n${licenses_content}`;
                if (package_manager.isGulpLocal) {
                    const templatesModules = JSON.stringify(
                        tplModulesToExport[path.basename(config.exportFile, '.js')]
                    );
                    extraContent = `\nSC.ENVIRONMENT.TEMPLATES_MODULE_NAMES = ${templatesModules};`;
                }
    ...
    stream = stream.pipe(chmod(0o666)).pipe(gulp.dest(dest, { mode: '0777' }));

    if (!package_manager.isGulpLocal && process.generateV2) {
        stream = stream.pipe(gulp.dest(destSS2, { mode: '0777' }));
    }
    ...
    gulp.src(requiredFiles, { allowEmpty: true })
        ...
        .pipe(
            map(function(file, cb) {
                ...
                const exposedModules = `SC.ENVIRONMENT.TEMPLATES_MODULE_NAMES = ${JSON.stringify(
                    tplModulesToExport[path.basename(config.exportFile, '.js')]
                )};`;
                file.contents = Buffer.from(`${content}\n${exposedModules}`);
                cb(null, file);
        ...
        .pipe(chmod(0o666))
        .pipe(gif(process.generateV2, gulp.dest(process.gulp_dest_ss2, { mode: '0777' })))
...
gulp.task('javascript-entrypoints', function(cb) {
    ...
    _(configs).each(function(config) {
        ...
        const content = `${contentPrefix}define('${dependenciesModuleName}', ${JSON.stringify(
            config.dependencies
        )}, function(${fn_params.join(',')}){ return Array.prototype.slice.call(arguments); }); `;
...
gulp.task('backward-compatibility-amd-unclean', function() {
    ...
    return gulp
        ...
        .pipe(
            gif(process.generateV2, gulp.dest(path.join(process.gulp_dest_ss2), { mode: '0777' }))
        );
});
...
gulp.task('libs', function libs() {
    function getAMDIndexFile(requiredFiles) {
        ...
        const indexContent = `define('${indexName}', [${_.map(requiredModules, function(m) {
            if (m.indexOf('handlebars.runtime') === 0) {
                return "'Handlebars'";
            }
            return `'${m}'`;
        }).join(', ')}], function(${_.map(requiredModules, function() {
            counter++;
            return `a${counter}`;
        }).join(', ')}){});`;
