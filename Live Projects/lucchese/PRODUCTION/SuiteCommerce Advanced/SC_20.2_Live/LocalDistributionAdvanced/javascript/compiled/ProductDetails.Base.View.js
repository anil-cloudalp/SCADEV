    ...
    var ProductDetailsBaseView = BackboneView.extend({
        ...
        bindModel: function bindModel() {
            ...
            this.model.areAttributesValid = _.wrap(this.model.areAttributesValid, function (fn, attributes, validators) {
                ...
                var current_validation = _.extend({}, self.model.validation);
                var attribute_validation_result;
                ...
                _.each(attributes, function (attribute) {
                    ...
                    if (attribute_validation_result) {
                        ...
                        BackboneExtras.Validation.callbacks.invalid(self, attribute, attribute_validation_result, 'name');
                    }
                    else {
        ...
        productDetails: function productDetails(api_query, options) {
            ...
            item.fetch({
                ...
                preventDefault: true
            }).then(function (data, _result, jqXhr) {
                if (!item.isNew()) {
            ...
            var option_bindings = self.model.get('options').reduce(function (bindings, option) {
                ...
                bindings["[name=\"" + cart_option_id + "\"]"] = {
                    ...
                    onSet: function (value, options) {
                        ...
                        var product_model = view.model;
                        var option = product_model
                            .get('options')
                            .findWhere({ cartOptionId: options.observe });
                        var current_value = option.get('value') && option.get('value').internalid;
                        if (!option.get('isMandatory') &&
                            current_value === value &&
                            ...
                            attributes: [
                                {
                                    ...
                                    observe: option.get('cartOptionId'),
                                    onGet: function () {
                                        var optionResult = _.findWhere(option.get('values'), { internalid: value.internalid });
                                        var isAvailable = !optionResult.isAvailable;
                                        if (isAvailable) {
                                            return 'muted';
                                        }
                                        if (value.internalid ===
        ...
        getBindingEventForOption: function getBindingEventForOption(option) {
            return (ProductDetailsBaseView.optionBindEventByType[option.get('type').toLowerCase()] ||
                'blur');
        },
        ...
        childViews: {
            ...
            'GlobalViewsMessageView.WronglyConfigureItem': function () {
                return new GlobalViews_Message_View_1.GlobalViewsMessageView({
                    message: Utils.translate('<b>Warning</b>: This item is not properly configured, please contact your administrator.'),
                    type: 'warning'
                });
            },
...
//# sourceMappingURL=ProductDetails.Base.View.js.map
