        ...
        var CMSAdapterComponent = SC_BaseComponent_1.SCBaseComponent.extend({
            ...
            updateContent: function updateContent(cctId, cctInstanceId, cctSelector, cctSettings, renderSettings) {
                ...
                if (rawCCTs) {
                    ...
                    var self_1 = this;
                    var cct_container_1 = customContentTypesViews[cctInstanceId] &&
                    ...
                    if (cct_container_1) {
                        if (renderSettings.position === customContentTypesViews[cctInstanceId].position) {
                            var placeholder = SC_BaseComponent_ChildViewsComponent_1.SCBaseComponentChildViewsComponent.getPlaceholderViews("[data-cms-area=\"" + cct_container_1 + "\"]");
                            var views = placeholder && placeholder.views;
                            _.each(views, function (view) {
                                var cctContainer = view.getChildViewInstance(cct_container_1, cctInstanceId);
                                if (cctContainer) {
            ...
            removeContent: function removeContent(cctInstanceId, destroyCall) {
                if (rawCCTs) {
                    ...
                    if (views) {
                        _.each(views, function (view) {
                            async_operations_2.push(view.removeChildViewInstance(container_1, cctInstanceId, destroyCall));
                        });
                        if (destroyCall) {
